...
(defconst json-mode-beautify-command-python2-with-unicode-escape
  "python2 -c \"import sys,json,collections; data=json.loads(sys.stdin.read(),object_pairs_hook=collections.OrderedDict); print json.dumps(data,sort_keys=%s,indent=2,separators=(',',': '))\"")
; The following Python 3 one-liner seems to contain bugs... Use Python2.
...
(eval-after-load "json-mode"
  '(defun json-mode-beautify (&optional preserve-key-order)
     ...
     (shell-command-on-region (if (use-region-p) (region-beginning) (point-min))
                              ...
                              (current-buffer) t)))
(eval-after-load "json-mode"
  '(defun json-mode-beautify-with-unicode-escape (&optional preserve-key-order)
     "Beautify / pretty-print from BEG to END, and optionally PRESERVE-KEY-ORDER."
     (interactive "P")
     (shell-command-on-region (if (use-region-p) (region-beginning) (point-min))
                              (if (use-region-p) (region-end) (point-max))
                              (concat (if (executable-find "env") "env " "")
                                      (format (if (executable-find "python2")
                                                  json-mode-beautify-command-python2-with-unicode-escape
                                                json-mode-beautify-command-python3-with-unicode-escape)
                                              (if preserve-key-order "False" "True")))
                              (current-buffer) t)))
