...
if [ $rc = 0 ]; then
  ...
  fisher_install_path="${HOME}/.config/fish/functions/fisher.fish"
  ...
  if [ $rc != 0 ]; then
    echo "Failed to install Fisher [$rc]"
    exit $rc
  fi

  echo "Fisher is successfully installed to [${fisher_install_path}]."
fi
